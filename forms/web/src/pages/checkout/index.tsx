...
const Checkout: Component = () => {
  ...
  const [submissionData] = createResource(() =>
    getFormSubmission(params.submission_id),
  );

  const getSessionList = () =>
    submissionData()?.order_items.filter(
      (item) => item.addon_type === AddonType.SESSION,
    );

  const getMealList = () =>
    submissionData()?.order_items.filter(
      (item) => item.addon_type === AddonType.MEAL,
    );
  ...
  const getEarlyDiscount = () =>
    submissionData()?.order_items.find(
      (item) => item.addon_type === AddonType.EARLY_DISCOUNT,
    );

  const getOnSitePrice = () =>
    Number(submissionData()?.order.amount) +
    Number(getEarlyDiscount()?.price || 0);

  const getSubtotal = () => {
    return (
      Number(submissionData()?.order.amount) +
      (getDiscountList()?.reduce(
  ...
  const handleCulqiAction = async () => {
    ...
    try {
      ...
      (window as any).Culqi3DS.initAuthentication(token);
      await updatePayment({
      ...
      notificationStore.error(
        (err as any).message || "No se pudo procesar el pago",
      );
    } finally {
      setLoading(false);
    }
  ...
  const openCulqi = async () => {
    ...
    (window as any).Culqi = new (window as any).CulqiCheckout(
      PUBLIC_KEY,
      config,
    );
  ...
  const checkout = async (typ: PaymentType) => {
    ...
    try {
      ...
      notificationStore.error(
        `No se pudo procesar el formulario: ${(err as any).message} `,
      );
    } finally {
      setLoading(false);
    }
      ...
      <div class="grid grid-cols-1 gap-4">
        <div role="alert" class="alert alert-info block">
          <p>
            ...
            {submissionData()?.submission.first_name +
              " " +
              submissionData()?.submission.last_name}
          </p>
          <p>
            <strong>Correo:</strong> {submissionData()?.submission.email}
          </p>
          <p>
            <strong>Telefono:</strong>
            {`(${submissionData()?.submission.country_code}) ${submissionData()?.submission.phone} `}
          </p>
        </div>
                  ...
                  <li class="list-row">
                    <div class="list-col-grow">
                      ...
                      <div class="text-sm text-base-content/50">
                        {getDateList(item.date_time)}
                      </div>
                    </div>
                    <div class="text-sm font-bold">
                      {getMoneyDisplay(item.currency, Number(item.price))}
                    </div>
                  </li>
                )}
              </For>
