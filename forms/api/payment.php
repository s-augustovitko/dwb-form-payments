...
try {
    ...
    // REQUIRED FIELDS
    // =========================
    $required_fields = ["payment_type", "submission_id"];
    ...
    if (
        !empty($input["payment_type"]) &&
        !in_array(
            $input["payment_type"],
            EnumConstants::getPaymentTypes(),
            true,
        )
    ) {
    ...
    if (empty($input["culqi_token"]) && $input["payment_type"] === "CULQI") {
        throw new Exception(
            "token invalido, por favor intente de nuevo mas tarde",
        );
    }
    ...
    if (empty($submission)) {
        throw new Exception(
            "no se encontro la entrada, por favor intente desde el paso anterior",
        );
    }
    ...
    try {
        ...
        $order = update_and_fetch_order_for_payment(
            ...
            $order_id,
            $submission["id"],
            $input["payment_type"] === "ON_SITE" ? "ON_SITE" : "DRAFT",
        );
    ...
    if ($input["payment_type"] === "CULQI" && (float) $order["amount"] > 0.0) {
        ...
        try {
            ...
            update_and_fetch_order_for_payment(
                ...
                $order["id"],
                $submission["id"],
                $payment_status === "FAILED" ? "CANCELLED" : "CONFIRMED",
            );
        ...
        if ($payment_status === "FAILED") {
            throw new Exception(
                "El pago no se pudo procesar: " . $user_message,
            );
        }
    }

    // Send success email if order is confirmed or on-site
    if (in_array($order["status"], ["CONFIRMED", "ON_SITE"], true)) {
        try {
            $items = fetch_order_items_by_order_id($order["id"]);
            $email_body = render_payment_email($submission, $order, $items);
            send_email(
                $submission["email"],
